...
$idd = $_SESSION['uid'] = $_GET['mid'];
$query = "SELECT * FROM tbl_theatre WHERE id = '$id'";
$res = mysqli_query($con, $query);
$row = mysqli_fetch_array($res);

$query = "SELECT * FROM tbl_login WHERE uid = '$id'";
$a = mysqli_query($con, $query);
$u = mysqli_fetch_array($a);
...
<script type="text/javascript" src="../../validation/dist/js/bootstrapValidator.js"></script>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-body">
            <form action="process_update_theater.php" method="post" id="form1">
              ...
              <div class="form-group">
                ...
                <input type="text" name="state" value="<?php echo $row['state']; ?>" id="administrative_area_level_1" s placeholder="State" class="form-control">
                <?php $frm->validate("state",array("required","label"=>"State")); // Validating form using form builder written in form.php ?>
              </div>
              <div class="form-group">
                <label class="control-label">Phone Number</label>
                <!-- <input type="number" name="phone number" id="autocomplete" class="form-control"> -->
                <input type="number" name="phno" value="<?php echo $row['phno']; ?>" class="form-control" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="10" title="Phone Number Must contain 10 digits and first letters should be 6 , 7 , 8 , or 9" required="">
                <?php $frm->validate("phno",array("required","label"=>"phno")); // Validating form using form builder written in form.php ?>
              </div>
              <div class="form-group">
                <label class="control-label">Email</label>
                <!-- <input type="number" name="phone number" id="autocomplete" class="form-control"> -->
                <input type="text" name="mail" value="<?php echo $row['mail']; ?>" class="form-control">
                <?php $frm->validate("mail",array("required","label"=>"mail")); // Validating form using form builder written in form.php ?>
              </div>


              <div class="form-group">
                <label class="control-label">Pin Code</label>
                 <input type="text" name="pin" value="<?php echo $row['pin']; ?>" id="postal_code"s placeholder="Zip code" class="form-control">
